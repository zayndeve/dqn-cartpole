...
class Config:
    """
    Configuration parameters for training a DQN agent.
    """
    ...
    start_learning_steps: int = 1000       # Number of steps before learning starts (warm-up)
    epsilon_frames: int = 20_000           # Steps over which ε decays linearly after warm-up

    # Learning parameters
    network_sync_rate: int = 500           # Steps between target network updates (hard sync)
    learning_rate_a: float = 1e-3
    discount_factor_g: float = 0.99        # Discount factor γ

    # Training stop criteria / model settings
...
def load_config(yaml_path: str, profile: str) -> Config:
    """
    Load hyperparameters from a YAML file for a specific profile.
    """
...
class Agent:
    """
    DQN agent with training and evaluation methods.
    Handles:
    - Environment setup
    - Policy and target network creation
    - Replay buffer
    ...
    def _norm(self, s: np.ndarray) -> np.ndarray:
        """
        Optional observation normalization for stability.
        Scales and clips states to [-1, 1] based on fixed bounds.
        """
    ...
    def optimize(self):
        """
        Sample a batch from the replay buffer and update the policy network.
        """
    ...
    def load(self, path: str):
        """
        Load model parameters and update target network.
        """
...
def main():
    ...
    parser.add_argument("--outdir", default="saved_models", help="Directory to save models/plots")
